...
<section class="page-title-section">
    <div class="title-banner">
        <img src="<?php echo @$meta->banner?base_url($meta->banner):base_url($brand_category->image); ?>" alt="title-banner" />
    </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-title-block text-center">
                        <h2><?php echo $meta->banner_title?$meta->banner_title:$meta->name; ?></h2>
...
<?php if (!empty($meta->top_title)): ?>
	
<section class="card_info">
    <div class="container-fluid">
        <div class="content">
            <h1><?php echo $meta->top_title; ?></h1>
            <p><?php echo $meta->top_des; ?></p>
            <?php if (!empty($meta->link)): ?>
            	  <a href="<?php echo $meta->link; ?>"><?php echo $meta->link; ?></a><br>
            <?php endif ?>
          
            <a href="<?php echo base_url($meta->btn_link);?>" class="theme-btn" ><?php echo $meta->btn_title?$meta->btn_title:'View OEM Catalogue'; ?></a>
        </div>
    </div>
</section>
<?php endif ?>
              ...
              <div class="col-xl-4 col-lg-6 col-sm-6">
                  <div class="product-block">
                      <div class="product-image">
                         <a href="<?php echo base_url('brand/'.$brand_category->seo_url.'/'.$brand->seo_url.'/'.$row['product_seo_url']); ?>"> <img src="<?php echo $row['image']?base_url($row['image']):base_url($config_logo); ?>" alt="<?php echo $row['name']; ?>" /></a>
                      ...
                      <div class="product-info">
                          <a href="<?php echo base_url('brand/'.$brand_category->seo_url.'/'.$brand->seo_url.'/'.$row['product_seo_url']); ?>"><h2><?php echo $row['name']; ?></h2></a>
                          ...
                          <a href="<?php echo base_url('brand/'.$brand_category->seo_url.'/'.$brand->seo_url.'/'.$row['product_seo_url']); ?>" class="theme-btn full">SHOP NOW</a>
                      </div>
                  </div>
              </div>
...
<?php if (!empty($meta->b_des)): ?>
	

<section class="card_info2">
    <div class="container-fluid">
        <div class="content">
        	<?php echo $meta->b_des; ?>
        </div>
...
<input type="hidden" name="brand_cat_seo_url"  value="<?php echo $brand_category->seo_url; ?>">
<input type="hidden" name="brand_seo_url"  value="<?php echo $brand->seo_url; ?>">
...
$('form#filter_form').submit(function(e){
     var type = $('#type').val();
      ...
      var form = $(this);
      $.ajax({
      url:"<?php echo base_url('get_brand_product_ajax'); ?>",
      type:"POST",
      data:form.serialize(),
      beforeSend:function(){
          if(type=='load'){
...
function load_first_data() {
    $.ajax({
        ...
        data: {
            "type": "load",
            "brand_id": "<?=$brand_id?>",
            "brand_cat_seo_url": "<?=$brand_category->seo_url; ?>",
            "brand_seo_url": "<?=$brand->seo_url; ?>",
            "offset": 0
        },
        dataType: 'json',
        success: function(obj) {
            ...
            if (obj.type === 'load') {
                ...
                $('#my__List').html(obj.ss); // it's filter data
            }
            $(".star-rating-box").each(function() {
                var rating = $(this).attr("data-rating");
                if (rating !== 'NAN') {
